...
audit:
  log:
    ...
    # 降级配置（可选，默认启用）
    fallback:
      enabled: true  # 是否启用降级功能
      directory: ./data/audit-fallback  # 降级文件存储目录
      
      # 以下配置会根据 logScene 自动设置，仅在需要自定义时配置
      rotation:
        max-lines: 10000  # 单个文件最大行数
        max-size: 104857600  # 单个文件最大大小（字节）
        max-age: 86400000  # 文件最大存活时间（毫秒）
      
      compensation:
        initial-interval: 5000  # 初始扫描间隔（毫秒）
        max-interval: 60000  # 最大扫描间隔（毫秒）
        batch-size: 100  # 每次补偿的批量大小
        read-lock-timeout: 3000  # 读取当前文件的锁超时时间（毫秒）
```
...
|--------|-----------|--------------|------------|
| **rotation.max-lines** | 5,000 | 10,000 | 50,000 |
| **rotation.max-size** | 50MB | 100MB | 500MB |
| **rotation.max-age** | 12小时 | 24小时 | 48小时 |
| **compensation.initial-interval** | 10秒 | 5秒 | 2秒 |
| **compensation.max-interval** | 2分钟 | 60秒 | 30秒 |
| **compensation.batch-size** | 50 | 100 | 200 |
| **compensation.read-lock-timeout** | 5秒 | 3秒 | 2秒 |

**配置原则：**
- ✅ **零配置开箱即用**：只需设置 `logScene`，fallback 会自动匹配最优配置
- ✅ **灵活覆盖**：如需自定义，只需覆盖需要修改的配置项
- ✅ **低频保守**：减少资源占用，降低扫描频率
- ✅ **高频激进**：快速响应，大批量补偿
...
当数据库异常时，审计日志会自动降级到本地文件存储，保证日志不丢失。数据库恢复后，补偿线程会自动将文件中的日志补偿回数据库。
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `fallback.enabled` | true | 是否启用降级功能 |
| `fallback.directory` | ./data/audit-fallback | 降级文件存储目录 |
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `rotation.max-lines` | 10000 | 单个文件最大行数 |
| `rotation.max-size` | 104857600 (100MB) | 单个文件最大大小（字节） |
| `rotation.max-age` | 86400000 (24小时) | 文件最大存活时间（毫秒） |
...
控制补偿线程的行为，将文件中的日志补偿回数据库。

| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `compensation.enabled` | true | 是否启用自动补偿 |
| `compensation.initial-interval` | 5000 (5秒) | 初始扫描间隔 |
| `compensation.max-interval` | 60000 (60秒) | 最大扫描间隔 |
| `compensation.batch-size` | 100 | 每次补偿的批量大小 |
| `compensation.read-lock-timeout` | 3000 (3秒) | 读取当前文件的锁超时时间 |

**动态扫描间隔机制：**
- 发现待补偿文件：使用 `initial-interval`（快速响应）
- 无待补偿文件：逐步增加间隔，最高到 `max-interval`（降低负载）
- 补偿失败：重置为 `initial-interval`（快速重试）
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `index.file` | .compensation_index | 索引文件名 |
| `index.flush-interval` | 5000 (5秒) | 索引刷盘间隔 |

**索引文件格式：**
```properties
# 文件名=已补偿行数
audit_20260417_001.log=150
audit_20260417_002.log=80
```
...
audit:
  log:
    logScene: high  # 高频场景，自动匹配激进配置
    fallback:
      enabled: true
      directory: /var/log/audit-fallback  # 独立磁盘
      # 以下配置会自动设置，无需手动配置
```
